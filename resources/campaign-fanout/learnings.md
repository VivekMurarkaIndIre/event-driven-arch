...
### 2026-06-08
- **JavaScript's single-threaded event loop makes a token bucket's `tryConsume` method inherently atomic.** Two concurrent async callers cannot interleave inside a synchronous function — each runs to completion before the other starts. This means the token decrement in `TenantRateLimiter.tryConsume` is safe without a mutex or lock, unlike the same code in a multi-threaded language where you'd need an atomic CAS or a mutex around the read-modify-write.

- **A semaphore's `release()` should hand the permit directly to the next waiter, not increment then decrement.** Calling `resolve()` on the waiting Promise hands control to its microtask queue entry without changing `this.permits`. Incrementing permits and then letting the waiter decrement it on its next `acquire()` call adds an unnecessary event-loop trip and briefly makes `this.permits` inconsistent (positive when a waiter is about to consume it). FIFO ordering and zero-gap hand-off require the direct hand-off pattern.

- **Token bucket `burstCapacity` determines how long a tenant can exceed the steady-state rate before being throttled.** With `maxRatePerSecond: 20` and `burstCapacity: 5`, a tenant can immediately process 5 messages (draining the full bucket), then is throttled to 1 message per 50 ms. Without a burst cap (`burstCapacity = maxRatePerSecond = 20`), a tenant that has been idle accumulates 20 tokens and can fire 20 messages before throttling — useful for bursty but low-average-rate workloads.

- **`maxConcurrentPerTenant` changes `processMessageBatch` from "batch of N" to "batch of 1."** When `BaseConsumer.poll()` fans out with `Promise.all`, it calls `processMessageBatch([msg])` once per message. Subclasses that work correctly on any array size (for…of loop, per-message idempotency check) are unaffected. Subclasses that rely on the full batch being available at once (e.g. building a DynamoDB `BatchWriteItem` request across all messages) break silently — they see one item instead of ten. This is the only backwards-incompatible constraint when enabling per-tenant limiting on an existing consumer.

- **Per-consumer-instance rate limiting scales with the consumer fleet.** If 3 replicas each enforce `maxRatePerSecond: 20`, tenant-a can saturate all 3 simultaneously for an effective 60 msg/s. This is often acceptable — the per-replica cap prevents one tenant from monopolising a single consumer's CPU/I/O — but it is not a global cap. True global rate limiting requires shared state (Redis, DynamoDB conditional decrement) with the associated latency and failure-mode complexity.
